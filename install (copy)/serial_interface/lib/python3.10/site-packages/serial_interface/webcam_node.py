...
class WebcamNode(Node):
    def __init__(self):
        ...
        self.alert_pub = self.create_publisher(String, '/webcam_alert', 10)  # ✅ NEW
    ...
    def timer_callback(self):
        ...
        # Détection de la pose (corps humain)
        ...
        if result.pose_landmarks:
            ...
            if not self.already_alerted:
                alert = String()
                alert.data = "BODY_DETECTED"
                self.alert_pub.publish(alert)
                self.get_logger().info("🛑 !!!BODY DETECTED — Published BODY_DETECTED")
            ...
            if self.already_alerted:
                ...
                alert.data = "CLEAR"
                self.alert_pub.publish(alert)
                self.get_logger().info("✅ No body — Published CLEAR")
                self.already_alerted = False
